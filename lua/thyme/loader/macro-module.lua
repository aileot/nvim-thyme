...
local function initialize_macro_searcher_on_rtp_21(fennel)
  ...
  local function _36_(...)
    local case_37_, case_38_ = search_fnl_macro_on_rtp_21(...)
    if (nil ~= case_37_) then
      local chunk = case_37_
      return chunk
...
local function hide_macro_cache_21(module_name)
  if (nil == module_name) then
    _G.error("Missing argument module-name on fnl/thyme/loader/macro-module.fnl:152", 2)
  else
  end
...
local function restore_macro_cache_21(module_name)
  if (nil == module_name) then
    _G.error("Missing argument module-name on fnl/thyme/loader/macro-module.fnl:159", 2)
  else
  end
