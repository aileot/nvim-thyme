...
local function prompt_fallback_config_21()
  ...
  if (case_3_ == 2) then
    ...
    if (case_4_ == 2) then
      ...
      local case_5_ = vim.fn.confirm(("Aborted trusting %s. Exit?"):format(config_path), "&No\n&yes", 1, "WarningMsg")
      if (case_5_ == 2) then
        return os.exit(1)
      else
        return nil
    ...
    local _ = case_3_
    local case_8_ = vim.fn.confirm("Aborted proceeding with nvim-thyme. Exit?", "&No\n&yes", 1, "WarningMsg")
    if (case_8_ == 2) then
      return os.exit(1)
    else
      return nil
