 ...
 local _local_2_ = require("thyme.utils.iterator") local each_file = _local_2_["each-file"]
 local _local_3_ = require("thyme.utils.pool") local hide_file_21 = _local_3_["hide-file!"]
 local _local_4_ = require("thyme.const") local state_prefix = _local_4_["state-prefix"]
 ...
 return modmap end module_maps = setmetatable({}, {__index = _5_})
 ...
 local module_map
 local function _6_() local modmap, logged_3f = ModuleMap.new(dependency["fnl-path"])
 if not logged_3f then modmap["set-module-map!"](modmap, dependency) else end

 module_maps[dependency["fnl-path"]] = modmap
 return modmap end module_map = (rawget(module_maps, dependency["fnl-path"]) or _6_())
 local _8_ = dependent_stack[#dependent_stack] if (nil ~= _8_) then local dependent = _8_
 ...
 local _11_ = fnl_path__3eentry_map(fnl_path) if (nil ~= _11_) then local modmap = _11_
 ...
 return each_file(hide_file_21, modmap_prefix) end
