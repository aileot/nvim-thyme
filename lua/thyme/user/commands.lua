...
local function define_commands_21(_3fopts)
  ...
  local function _29_()
    ...
    for _, path in ipairs(files) do
      ...
      if directory_3f(path) then
        local _30_ = vim.fn.delete(path, "rf")
        if (_30_ == 0) then
  ...
  vim.api.nvim_create_user_command("ThymeUninstall", _29_, {desc = "[thyme] delete all the thyme's cache, state, and data files"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "EvalFile"), _36_, {range = "%", nargs = "?", complete = "file", desc = "[thyme] evaluate given file, or current file, and display the results"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "EvalBuffer"), _39_, {range = "%", nargs = "?", complete = "buffer", desc = "[thyme] evaluate given buffer, or current buffer, and display the results"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "CompileBuffer"), _43_, {range = "%", nargs = "?", complete = "buffer", desc = "[thyme] display the compiled lua results of current buffer"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "CompileFile"), _46_, {nargs = "*", bang = true, complete = "file", desc = "Compile given fnl files, or current fnl buffer"})
  local function _66_(_64_)
    ...
    do
      ...
      if (_67_ == ".fnl") then
        local _68_ = fnl_path__3elua_path(input_path)
        if (nil ~= _68_) then
          local lua_path = _68_
  ...
  return vim.api.nvim_create_user_command((fnl_cmd_prefix .. "Alternate"), _66_, {nargs = "?", complete = "file", desc = "[thyme] alternate fnl<->lua"})
end
