...
local _local_3_ = require("thyme.wrapper.treesitter")
local inject_vim_cmd_arg_query_21 = _local_3_["inject-vim-cmd-arg-query!"]
local _local_4_ = require("thyme.user.command.fennel.fennel-wrapper")
local parse_cmd_buf_args = _local_4_["parse-cmd-buf-args"]
local parse_cmd_file_args = _local_4_["parse-cmd-file-args"]
local mk_fennel_wrapper_command_callback = _local_4_["mk-fennel-wrapper-command-callback"]
local fnl_file_compile = require("thyme.user.command.fennel.fnl-file-compile")
local M = {}
local function open_buf_21(buf_7cpath, _5_)
  local split = _5_["split"]
  local tab = _5_["tab"]
  local mods = _5_
  local split_3f = ((-1 ~= tab) or ("" ~= split))
  local cmd
  do
    local _6_ = type(buf_7cpath)
    if (_6_ == "number") then
      if split_3f then
        cmd = "sbuffer"
      else
...
M["setup!"] = function()
  ...
  if not Config["disable-treesitter-highlights"] then
    inject_vim_cmd_arg_query_21("Fnl", "fennel")
    inject_vim_cmd_arg_query_21("FnlCompile", "fennel")
  else
  end
  ...
  local function _16_(_14_)
    ...
    local mods = _14_["smods"]
    local input_path = vim.fn.expand((_3fpath or "%:p"))
    local output_path
    do
      local _17_ = input_path:sub(-4)
      if (_17_ == ".fnl") then
        local _18_ = DependencyLogger["fnl-path->lua-path"](DependencyLogger, input_path)
        if (nil ~= _18_) then
          local lua_path = _18_
          output_path = lua_path
        else
          local _ = _18_
          local _19_ = (input_path:sub(1, -4) .. "lua")
          if (nil ~= _19_) then
            local lua_path = _19_
  ...
  return vim.api.nvim_create_user_command("FnlAlternate", _16_, {nargs = "?", complete = "file", desc = "[thyme] alternate fnl<->lua"})
end
return M
