...
local function compile_to_write_21(fnl_path, force_compile_3f)
  ...
  if (0 == #fnl_path) then
    ...
    fnl_paths = vim.fn.flatten(_3_, 1)
  end
  local path_pairs
  do
    local tbl_16_ = {}
  ...
  if not or_7_ then
    ...
    if and_11_ then
      if (0 < #existing_lua_files) then
        local _12_ = vim.fn.confirm(("The following files have already existed:\n" .. table.concat(existing_lua_files, "\n") .. "\nOverride the files?"), "&No\n&yes")
        if (_12_ == 2) then
          and_11_ = true
        else
          local _ = _12_
...
local function create_commands_21()
  ...
  local function _22_(_20_)
    local fargs = _20_["fargs"]
    local should_write_file_3f = _20_["bang"]
    local _arg_21_ = _20_["mods"]
    local confirm_3f = _arg_21_["confirm"]
    local function _23_()
      if (0 == #fargs) then
