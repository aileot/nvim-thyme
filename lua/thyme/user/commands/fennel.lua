...
local function edit_cmd_history_21(new_fnl_code, opts)
  ...
  local _13_ = methods[opts.method]
  if (nil ~= _13_) then
...
local function open_buffer_21(buf_7cpath, _23_)
  local split = _23_["split"]
  local tab = _23_["tab"]
  local mods = _23_
  local split_3f = ((-1 ~= tab) or ("" ~= split))
  local cmd
  do
    local _24_ = type(buf_7cpath)
    if (_24_ == "number") then
      if split_3f then
        cmd = "sbuffer"
      else
...
M["setup!"] = function(_3fopts)
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "EvalFile"), _32_, {range = "%", nargs = "?", complete = "file", desc = "[thyme] evaluate given file, or current file, and display the results"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "EvalBuffer"), _35_, {range = "%", nargs = "?", complete = "buffer", desc = "[thyme] evaluate given buffer, or current buffer, and display the results"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "CompileBuffer"), _39_, {range = "%", nargs = "?", complete = "buffer", desc = "[thyme] display the compiled lua results of current buffer"})
  ...
  vim.api.nvim_create_user_command((fnl_cmd_prefix .. "CompileFile"), _42_, {nargs = "*", bang = true, complete = "file", desc = "Compile given fnl files, or current fnl buffer"})
  ...
  return vim.api.nvim_create_user_command((fnl_cmd_prefix .. "Alternate"), _62_, {nargs = "?", complete = "file", desc = "[thyme] alternate fnl<->lua"})
end
return M
