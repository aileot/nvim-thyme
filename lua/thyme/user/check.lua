...
local fnl_path__3elua_path = _local_3_["fnl-path->lua-path"]
local fnl_path__3eentry_map = _local_3_["fnl-path->entry-map"]
local fnl_path__3edependent_map = _local_3_["fnl-path->dependent-map"]
local clear_module_map_21 = _local_3_["clear-module-map!"]
local restore_module_map_21 = _local_3_["restore-module-map!"]
local _local_4_ = require("thyme.searcher.module")
...
local function fnl_path__3edependent_count(fnl_path)
  local _6_ = fnl_path__3edependent_map(fnl_path)
  if (nil ~= _6_) then
    local dependent_map = _6_
    local i = 0
...
local function recompile_21(fnl_path, lua_path, module_name)
  ...
  local _8_, _9_ = Observer["observe!"](Observer, fennel["compile-string"], fnl_path, lua_path, compiler_options, module_name)
...
local function update_module_dependencies_21(fnl_path, _3flua_path, opts)
  ...
  local _let_11_ = fnl_path__3eentry_map(fnl_path)
  local module_name = _let_11_["module-name"]
  if _3flua_path then
