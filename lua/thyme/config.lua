...
local contains_3f = _local_2_["contains?"]
...
default_opts_main_only = {preproc = _4_, notifier = {recompile = vim.notify, ["fennel-update"] = vim.notify, reload = false}}
if not file_readable_3f(config_path) then
  ...
  if (_5_ == 2) then
    ...
    vim.defer_fn(_6_, 800)
  end
else
end
local function find_config_file(path)
  local _12_ = vim.fs.find(config_filename, {upward = true, type = "file", stop = uv.os_homedir(), path = path})
  if ((_G.type(_12_) == "table") and (nil ~= _12_[1])) then
    local project_config_path = _12_[1]
...
local function read_config(config_file_path)
  ...
  local config_table
  do
    local _14_ = cache["config-list"][config_file_path]
    local function _15_()
      local _3fcache = _14_
      return ((nil == _3fcache) or (_3fcache.mtime.sec < fs_stat.mtime.sec))
    end
    if (true and _15_()) then
      local _3fcache = _14_
      local secure_config_3f = contains_3f(secure_config_paths, config_file_path)
      local config_lines
      ...
      config_table = config
    elseif ((_G.type(_14_) == "table") and (nil ~= _14_.config)) then
      local config = _14_.config
      config_table = config
    else
      config_table = nil
    end
...
local function _18_()
  ...
  return (cache["main-config"] or _19_())
end
get_main_config = _18_
local function config_file_3f(path)
  return (config_filename == vim.fs.basename(path))
end
local function get_option_value(config, key)
  _G.assert((nil ~= key), "Missing argument key on fnl/thyme/config.fnl:126")
  _G.assert((nil ~= config), "Missing argument config on fnl/thyme/config.fnl:126")
