...
local function find_config_file(path)
  local _11_ = vim.fs.find(config_filename, {upward = true, type = "file", stop = uv.os_homedir(), path = path})
  if ((_G.type(_11_) == "table") and (nil ~= _11_[1])) then
    local project_config_path = _11_[1]
...
local function read_config(config_file_path)
  ...
  local config_table
  do
    local _13_ = cache["config-list"][config_file_path]
    local function _14_()
      local _3fcache = _13_
      return ((nil == _3fcache) or (_3fcache.mtime.sec < fs_stat.mtime.sec))
    end
    if (true and _14_()) then
      local _3fcache = _13_
      local config_lines
      ...
      config_table = config
    elseif ((_G.type(_13_) == "table") and (nil ~= _13_.config)) then
      local config = _13_.config
      config_table = config
    else
      config_table = nil
    end
...
local function _17_()
  ...
  return (cache["main-config"] or _18_())
end
get_main_config = _17_
