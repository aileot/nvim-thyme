...
local function read_config_with_backup_21(config_file_path)
  ...
  local mounted_backup_path = backup_handler["module-name->mounted-backup-path"](backup_handler)
  local config_code
  if file_readable_3f(mounted_backup_path) then
    local msg = ("[thyme] rollback config to mounted backup (created at %s)"):format(backup_handler["module-name->active-backup-birthtime"](backup_handler))
  ...
  if ok_3f then
    ...
    if backup_handler["should-update-backup?"](backup_handler, config_code) then
      backup_handler["create-module-backup!"](backup_handler, config_file_path)
