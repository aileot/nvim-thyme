...
(describe* "rollback"
  ...
  (it* ".create-module-backup! creates a backup file."
    (let [kind "foo"
          ...
          rollback-handler (rollback-manager:handlerOf module-name)
          stored-path (rollback-handler:module-name->active-backup-path)
      ...
      (->> (vim.fn.filereadable stored-path)
           (assert.is_same 0))
      (rollback-handler:create-module-backup! original-path)
      (->> (vim.fn.filereadable stored-path)
           (assert.is_same 1)))))
  ...
  (let [default-max-rollbacks config.max-rollbacks]
    ...
    (it* "limits the number of backups per module to `config.max-rollbacks`."
      (let [mod :foobar
            ...
            rollback-handler (TestRollbackManager:handlerOf)]
        (assert.equals 0 (length (rollback-handler:module-name->backup-files)))
        (rollback-handler:create-module-backup! path)
        ...
        (vim.wait 1)
        (rollback-handler:create-module-backup! path)
        (assert.equals 4 (length (rollback-handler:module-name->backup-files)))
        (prepare-config-fnl-file! filename "ctx5")
        (vim.wait 1)
        (rollback-handler:create-module-backup! path)
        (assert.equals 5 (length (rollback-handler:module-name->backup-files)))
